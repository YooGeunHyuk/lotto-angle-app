...
- 추첨 전 회차는 `추첨전` 상태로 표시
- QR 버튼을 누르면 카메라 스캐너가 열린다.
- QR 스캔 후 동행복권 `winQr` URL 형식으로 해석되면 `QR 등록` 티켓으로 자동 저장된다.
- QR은 실제 한국 로또 QR 샘플을 받은 뒤 파서를 최종 보정할 예정이다.
- QR 해석 실패 시 스캔 원문을 화면에 표시해 샘플 확보와 디버깅이 가능하게 했다.
