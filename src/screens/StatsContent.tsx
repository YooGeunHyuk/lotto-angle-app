    ...
    <View style={s.safe}>
      ...
      >
        ...
        <View style={s.card}>
          ...
          <Row label="핵심 범위 (50%)" value={`${sumStats.p10} ~ ${sumStats.p90}`} sub="전체 당첨의 50% 구간" />
          <Row label="전체 범위" value={`${sumStats.min} ~ ${sumStats.max}`} />
        </View>

        {/* 홀짝 */}
        <View style={s.card}>
          <Text style={s.cardTitle}>홀수 / 짝수 분포</Text>
          {oddEvenSorted.map(([k, c]) => {
...
const s = StyleSheet.create({
  ...
  statRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 7, borderTopWidth: 1, borderTopColor: C.border },
  ...
  seasonRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderTopWidth: 1, borderTopColor: C.border, gap: 10 },
  seasonName: { width: 36, fontSize: 12, color: C.gray },
  row: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  freqRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 5, gap: 8, borderTopWidth: 1, borderTopColor: C.border },
